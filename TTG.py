...
def MakeTile(shadesNum, colors):
    print("\nStarted!")
    template = np.array(Image.open(resource_path(textype.get())).convert("RGBA"))
    ...
    template = Image.fromarray(np.array(template).astype('uint8'))
...
def getColor():
    ...
    hexcolor = color[1][1:]
    global colors
    colors = [[int(hexcolor[0:2], 16), int(hexcolor[2:4], 16), int(hexcolor[4:6], 16), 255]]
...
Button(root, text="Generate!", command=lambda: MakeTile(shadesNum.get(), colors)).grid(row=5, column=0, padx=10, pady=10)
